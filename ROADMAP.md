# ROADMAP — Melhorias futuras PFatu
...
1. src/lib/export.ts
   ...
   export function exportToExcel(
     ...
     XLSX.writeFile(wb, `PFatu_${month}.xlsx`);
   }
...
2. layout.tsx — script anti-flash (antes de qualquer CSS carregado):
   <head>
     <script dangerouslySetInnerHTML={{ __html: `
       (function(){
         const t = localStorage.getItem('pfatu-theme') || 'dark';
...
3. src/lib/use-theme.ts — hook simples:
   export function useTheme() {
     ...
     useEffect(() => {
       const saved = localStorage.getItem('pfatu-theme') as 'dark'|'light' || 'dark';
       setTheme(saved);
     }, []);
     function toggle() {
